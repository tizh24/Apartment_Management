...
export class ContractsCommandService {
  ...
  async create(dto: CreateContractDto, user: AuthenticatedRequestUser) {
    ...
    return this.contractsRepository.transaction(async (transaction) => {
      ...
      await this.syncSaleContractIfNeeded(transaction, contract.contractCode, contract);
  ...
  async update(id: string, dto: UpdateContractDto, user: AuthenticatedRequestUser) {
    ...
    await this.assertSaleContractCanBeChanged(existing, dto);
    ...
    return this.contractsRepository.transaction(async (transaction) => {
      ...
      await this.syncSaleContractIfNeeded(transaction, existing.contractCode, updated);
  ...
  private async syncSaleContractIfNeeded(
    transaction: ContractsTransactionClient,
    previousContractCode: string,
    contract: { contractCode: string; saleProfileId: string | null; apartmentId: string; roomId: string; customer: { fullName: string; phoneNumber: string }; startDate: Date; endDate: Date; monthlyRent: Prisma.Decimal; commissionAmount: Prisma.Decimal | null; status: LeaseContractStatus },
  ): Promise<void> {
    this.assertSaleCommissionReady(contract.saleProfileId, contract.commissionAmount);

    if (!contract.saleProfileId || !contract.commissionAmount) {
      return;
    }

    await this.contractsRepository.syncSaleContractFromLease(transaction, previousContractCode, {
      ...
      contractStatus: this.getSaleContractStatus(contract.status),
    });
  }

  private async assertSaleContractCanBeChanged(
    existing: { contractCode: string; saleProfileId: string | null; commissionAmount: Prisma.Decimal | null },
    dto: UpdateContractDto,
  ): Promise<void> {
    const saleContract = await this.contractsRepository.findSaleContractByLeaseContractCode(existing.contractCode);

    if (saleContract?.commissionStatus !== CommissionStatus.PAID) {
      return;
    }

    const saleChanged =
      dto.saleProfileId !== undefined && dto.saleProfileId !== existing.saleProfileId;
    const commissionChanged =
      dto.commissionAmount !== undefined &&
      !new Prisma.Decimal(dto.commissionAmount).equals(existing.commissionAmount ?? 0);

    if (saleChanged || commissionChanged) {
      throw new BadRequestException("Paid sale commission cannot be changed");
    }
  }

  private assertSaleCommissionReady(
    saleProfileId: string | null,
    commissionAmount: Prisma.Decimal | null,
  ): void {
    if (saleProfileId && !commissionAmount) {
      throw new BadRequestException("Commission amount is required when sale profile is provided");
    }
  }

  private getSaleContractStatus(status: LeaseContractStatus): SaleContractStatus {
    if (status === LeaseContractStatus.CANCELED) {
      return SaleContractStatus.CANCELED;
    }

    if (status === LeaseContractStatus.ENDED) {
      return SaleContractStatus.COMPLETED;
    }

    return SaleContractStatus.ACTIVE;
  }
